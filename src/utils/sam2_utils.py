...
def mask_first_frame_interactive(predictor, video_path, frame_idx = 0, viz=False):
    img = Image.open(os.path.join(video_path, f"{frame_idx:05}.jpg")).convert("RGB")
...
def mask_first_frame(predictor,
    ...
    """
    # Load image
    img_pil = Image.open(os.path.join(video_path, f"{frame_idx:05}.jpg")).convert("RGB")
...
def refine_masks_with_complement(
    ...
    p_coords = sample_points_in_mask(comp_mask, 10)
    p_coords = p_coords[:, None, :]
    p_labels = np.ones((p_coords.shape[0], p_coords.shape[1]))
    new_masks, scores, _ = img_predictor.predict(
        point_coords=p_coords,
        point_labels=p_labels,
        multimask_output=False,
    )
    # filter out duplicates via IoU
    kept = []
    for nm in new_masks:
        seg = nm
        # compute IoU vs each existing mask
        ious = [
            (np.logical_and(seg, m['segmentation'])).sum() /
            ((np.logical_or(seg, m['segmentation'])).sum() + 1e-8)
            for m in masks
        ]
        if max(ious, default=0) < min_iou_new:
            kept.append({"segmentation": nm,
                            "point_coords": [[int(p_coords[0,0,0]), int(p_coords[0,0,1])]]})
