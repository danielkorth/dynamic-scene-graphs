...
if __name__ == "__main__":
    ...
    print("Testing SALAD constraint handling:")
    print("=" * 60)
    print("Constraint: (H×W)/(14×14) > 64  =>  H×W > 12544")
    print("=" * 60)
    
    # Test the constraint function with various sizes
    test_sizes = [
        (224, 224),    # Standard size
        (100, 100),    # Too small
        (64, 64),      # Very small
        (300, 200),    # Rectangular
        (150, 400),    # Tall rectangle
    ...
    for w, h in test_sizes:
        print(f"\nTesting {w}x{h}:")
        new_w, new_h = salad._get_valid_salad_size(w, h)
        area = new_w * new_h
        patches = area / (14 * 14)
        
        print(f"  Result: {new_w}x{new_h} | Area: {area} | Patches: {patches:.1f}")
        print(f"  Constraints: Patches > 64? {'✓' if patches > 64 else '✗'} | Div by 14? {'✓' if new_w % 14 == 0 and new_h % 14 == 0 else '✗'}")
    ...
    try:
        ...
        features = salad.extract_features(image_path)
        print("SALAD features shape:", features.shape)
        print("Features type:", type(features))
            
    except FileNotFoundError:
        print(f"Image file not found: {image_path}")
        print("Please update the image_path to test the SALAD model.")
